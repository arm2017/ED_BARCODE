...
loadingApp
		.controller(
				...
				function($scope, $http) {
					...
					var contents = rw.readFileSync("userdata/proudcts.cfg",
							"utf8");
					var Response = rw.readFileSync("userdata/Response.txt",
							"utf8");
					...
					for (var i = 0; i < countGoodsList; i++) {
						...
						var Product = {};
						Product["LicenseName"] = $xmlService.getVal(
								"LicenseName", i);
						Product["LicenseNo"] = $xmlService.getVal("LicenseNo",
								i);
						Product["EffectiveDate"] = $xmlService.getVal(
								"EffectiveDate", i);
						Product["ExpireDate"] = $xmlService.getVal(
								"ExpireDate", i);
						...
						for (var j = loopCount; j < c + loopCount; j++) {
							var n = $xmlService.getVal("GoodsDescriptionText",
									j);
							...
							strItem = strItem.concat(
									$.trim($xmlService.getVal(
											"ProductTypeDescriptionText", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal(
											"GoodsDescriptionText", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal("BrandName", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService
											.getVal("SubbrandName", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal("GoodsSize", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal("Degree", j)))
									.concat("|");

							var goodsprice = $.trim($xmlService.getVal(
									"GoodsPrice", j));
							goodsprice = parseFloat(goodsprice).toFixed(4);
							strItem = strItem.concat($.trim(goodsprice))
									.concat("|");

							var dePrice = ($xmlService
									.getVal("DeclarePrice", j) == "") ? "0.0000"
							...
							strItem = strItem.concat(
									$.trim($xmlService.getVal(
											"ProductTypeCode", j))).concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal(
											"TaxRateByPriceAmount", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal(
											"TaxRateByQuantityAmount", j)))
									.concat("|");
							strItem = strItem.concat(
									$.trim($xmlService.getVal("DegreeMin", j)))
									.concat("|");
					...
					addressFactory += " ต. " + ThambolName + " อ. "
							+ AmphurName + " จ. " + ProvinceName + " "
							+ Postcode;
					if (TelNumber != "") {
